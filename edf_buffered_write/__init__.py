"""edf-buffered-write: buffered EDF/BDF writing for real-time (streaming) acquisition.
...
The class wraps any writer that exposes the pyedflib ``EdfWriter`` interface
(``channels``, ``get_smp_per_record``, ``writeSamples``, ``close``); it does not
depend on pyedflib internals and therefore also works with API-compatible
writers.
"""
...
class BufferedEdfWriter:
    ...
    def flush(self, pad_with_last=True):
        ...
        the default) rather than zero, so that no step discontinuity to zero is
        introduced. Note that, for a zero-mean AC biopotential, holding the last
        value still leaves a short constant (DC) segment at the very end of the
        recording; it merely avoids the sharp step that zero-padding would cause.
        """
