"""Tests for the reusable BufferedEdfWriter API shipped in this package.

These exercise the installable library (``edf_buffered_write``) directly, as a
user would call it, and complement ``test_roundtrip.py`` (which characterises the
naive-vs-buffered behaviour through the harness).
"""
...
def test_write_samples_rejects_wrong_signal_count(tmp_path):
    fs = 1000
    p = str(tmp_path / "bad.edf")
    w = pyedflib.EdfWriter(p, 1)
    w.setSignalHeader(0, _header(fs))
    w.setDatarecordDuration(1.0)
    bw = BufferedEdfWriter(w)
    with pytest.raises(ValueError):
        bw.write_samples([np.zeros(100), np.zeros(100)])  # 2 signals, writer has 1
    w.close()
